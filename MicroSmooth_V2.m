...
% Store original labels before smoothing (added by Ehsan Eqlimi, Sept 2025)
function [L,RawLabels] = MicroSmooth_V2(X, A, smooth_type, opts)
%% Initialisation
[C,N,T] = size(X); 
L = nan(T,N);
...
function [L,RawLabels] = reject_segments(X,A,opts)
...
% minTime (in samples).
% Store original labels before smoothing (added by Ehsan Eqlimi)
%% Initialisation
[C,N] = size(X);
K = size(A,2);
...
[~ ,labels] = sort(GMD,1);
RawLabels=labels;  % Store original labels before smoothing (added by Ehsan Eqlimi)
